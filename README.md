...
the radio stripped, which [`ysf_frame.py`](#building-a-ysf-frame) does.
...
`ysf_frame.py` holds the encoders the uplink needs: the FICH with its
Golay blocks, CRC and convolutional code, the data channel, and the
header and terminator payloads.
